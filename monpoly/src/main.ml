...
exception NotPastOnly
exception NotMonitorable
exception NotClosed

let main (module Log: LOG) (module Algorithm: ALGORITHM) =
  ...
  if !displayver then
    ...
    if !sigfile = "" then
      ...
      begin
        ...
        if ((not is_mon) && !Misc.databank) then raise NotMonitorable;
        if ((is_future f) && !Misc.databank) then raise NotPastOnly;
        if ((free_vars f != []) && !Misc.databank) then raise NotClosed;
        if !sigout then
          Predicate.print_vartypes_list vartypes
        else if is_mon && not !Misc.checkf then
          begin
            if not !nofilterrelopt then
              Algorithm.Filter_rel.enable pf;
            ...
            if !testfilteropt then
              ...
              (* start monitoring *)
              else if !Misc.resumefile <> "" then
                Algorithm.resume !logfile
              else if !Misc.combine_files <> "" then
...
let _ =
  Arg.parse [
    ...
    "-labels", Arg.Set Misc.labels, "\tUse labels";
    "-databank", Arg.Set Misc.databank, "\tConfigurate checking for Databank formulae";
    ...
    "-load", Arg.Set_string Misc.resumefile, "\tLoad monitor state from file";
    "-combine", Arg.Set_string Misc.combine_files, "\tComma separated partition files to combine";
  ...
  let module Log = (val (if !Misc.labels then (module Log.LabelledLog) else (module Log.StandardLog)): Log.LOG) in
  let module Algorithm = MakeAlgorithm(Log) in

  if Misc.debugging Dbg_perf then
    ignore(Unix.alarm 600);
  main (module Log) (module Algorithm);
  if !memarg then
