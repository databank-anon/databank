...
module type FILTER_REL = sig

  module Helper: HELPER

  val enable: MFOTL.formula -> unit
  val rel_OK: string -> bool
  val tuple_OK: Predicate.var -> Helper.Db.Tuple.tuple -> bool
...
module MakeFilter_rel (H: HELPER) = struct
    ...
    let rec process_is_cst pred tuple filter_is_cst filter_csts =
      ...
      | (p,i) :: filter_tail ->
         if p = pred then
           ...
           process_is_cst pred tuple filter_tail filter_csts
    in
    process_is_cst pred (H.Db.Tuple.get_constants tuple) !tuple_filter_is_cst !tuple_filter_csts
                   
  let debug_tuple_csts_OK pred tuple =
    let res = tuple_OK pred tuple in
    Printf.printf "filter: pred %s tuple" pred;
    H.Db.Tuple.print_tuple tuple;
    Printf.printf " ->";
    if res
