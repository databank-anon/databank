...
module MakeSplitting (M: MFORMULA) = struct
 ...
 - Essentially just call the above combination functions and return the combined state
  ...
  let combine_agg  agg1 agg2 =
    ...
    {tw_rels = agg1.tw_rels; other_rels = other_rels; mset = agg1.mset; hres = agg1.hres;
     labs = Labels.LabelSet.union agg1.labs agg2.labs}

  let combine_aggMM agg1 agg2 =
    let non_tw_rels = combine_queues2 agg1.non_tw_rels agg2.non_tw_rels in
    {non_tw_rels = non_tw_rels; tbl = agg1.tbl; labs = Labels.LabelSet.union agg1.labs agg2.labs}
  ...
  let free_vars2 f1 f2 =
    let vars1 = (free_vars f1) in
    let vars2 = (free_vars f2) in
  ...
  let split_state mapping mf size =
    ...
    let split_agg  agg p =
      ...
      Array.map (fun e -> { agg with other_rels = e }) res
    in
    let split_aggMM agg p =
      let res = split_queue2 agg.non_tw_rels p in      
      Array.map (fun e -> { agg with non_tw_rels = e }) res
    in
  ...
  let split_formula sconsts mf =
    ...
    split_state (fun x -> mapping (Tuple.get_constants x)) mf numparts
