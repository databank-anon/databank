...
module MakeHypercube_slicer (M: MFORMULA) = struct
  ...
  let dimensions formula = List.length (M.free_vars formula) 
  ...
  let add_slices_of_valuation slicer tuple free_vars =
    let tuple = Array.of_list (Tuple.get_constants tuple) in
    let free_vars_full = M.free_vars slicer.formula in
  ...
  let convert_heavy formula heavy_unproc =
    let preds = M.predicates formula in
    let variables_in_order = build_var_ids_preds (M.free_vars formula) preds !Domain_set.predicates in
