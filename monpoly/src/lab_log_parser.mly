...
%token AT LPA RPA LCB RCB LSB RSB COM TR LABELS NULL
%token <string> STR
%token EOF
%token CMD
%token EOC
...
%start tsdb
%type <(Log_parser_helper.LabelledLog_parser_helper.Helper.parser_feed)> tsdb	
...
/*      | CMD slicing_test EOC    { $2 }*/
      | AT STR db labels TR     { f "tsdb(next)"; DataTuple { ts = MFOTL.ts_of_string "Lab_log_parser" $2; db = H.make_db ~labels:$4 $3; complete = true } }
      | AT STR db labels AT     { f "tsdb(next)"; DataTuple { ts = MFOTL.ts_of_string "Lab_log_parser" $2; db = H.make_db ~labels:$4 $3; complete = false } }
      | AT STR db labels CMD    { f "tsdb(next)"; DataTuple { ts = MFOTL.ts_of_string "Lab_log_parser" $2; db = H.make_db ~labels:$4 $3; complete = false } }
      | AT STR db labels EOF    { f "tsdb(last)"; DataTuple { ts = MFOTL.ts_of_string "Lab_log_parser" $2; db = H.make_db ~labels:$4 $3; complete = false } }
...
table:
      | STR relation            { f "table";
                                  try
                                    H.make_table $1 ~labels:(snd $2) (fst $2)
...
relation:
      | tuple relation          { f "relation(list)"; $1::(fst $2), snd $2 }
      | LABELS labels           { f "relation(end+labels)"; [], $2 }
      |                         { f "relation(end)"; [], [] }

tuple:
      | LPA tuple_fields RPA    { f "tuple"; $2 }


tuple_fields:
      | NULL labels COM tuple_fields
                                { f "fields(list+labels)"; (None,$2)::$4 }
      | STR labels COM tuple_fields
                                { f "fields(list+labels)"; (Some $1,$2)::$4 }
      | NULL labels             { f "fields(end+labels)"; [None,$2] }	
      | STR labels              { f "fields(end+labels)"; [Some $1,$2] }
      |                         { f "fields()"; [] }
...
labels:
      | LSB cslabels RSB        { f "labels(list)"; $2 }
      |                         { f "labels()"; [] }

cslabels:
      | STR COM cslabels        { f "cslabels(list)"; $1::$3 }
      | STR                     { f "cslabels(end)"; [$1] }
