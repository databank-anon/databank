...
module type LOG = sig
  ...
  val get_next_entry : Lexing.lexbuf -> H.monpoly_feed
  val tp : int ref
...
module StandardLog = struct

  module H = StandardHelper
  ...
  let get_next_entry lexbuf =
    let rec get_next lexbuf =
      ...
      | DataTuple {ts; db; complete} ->
         ...
         last_ts := ts;
         incr tp;
         (*if !Filter_empty_tp.enabled && StandardHelper.Db.is_empty db then
           begin (* skip empty db *)
             incr skipped_tps;
             get_next lexbuf
           end
         else*) (* removed as this risked erasing labels *)
           StandardHelper.MonpolyData {tp = !tp - 1; ts; db}
      ...
      | ErrorTuple s ->
         ...
         if not !last && !Misc.new_last_ts then
           begin
             last := true;
             MonpolyData { tp = !tp; ts = MFOTL.ts_max; db = StandardHelper.Db.make_db [] }
           end
         else
           MonpolyError s
...
module LabelledLog = struct

  module H = LabelledHelper

  include CommonLog
               
  let get_signature_lexbuf lexbuf =
    try
      let sign = Lab_log_parser.signature Lab_log_lexer.token lexbuf in
      if Misc.debugging Dbg_all then
        Printf.eprintf "[Log.get_sign] The signature file was parsed correctly.\n";
      if (List.mem_assoc "tp" sign || List.mem_assoc "ts" sign || List.mem_assoc "tp" sign) then
        failwith "[Log.get_sign] The predicates tp, ts, tpts are predefined and \
  ...
  let get_next_entry lexbuf =
    let rec get_next lexbuf =
      let log_entry =
        try
          Lab_log_parser.tsdb Lab_log_lexer.token lexbuf
        with e ->
          let lnum, snum, cnum, token, line = show_error lexbuf in
          Printf.eprintf
            "[Log.get_next_entry] Failed to parse log file. \
      ...
      | DataTuple {ts; db; complete} ->
         ...
         LabelledHelper.MonpolyData {tp = !tp - 1; ts; db}
      ...
      | ErrorTuple s ->
         ...
         if not !last && !Misc.new_last_ts then
           begin
             last := true;
             MonpolyData { tp = !tp; ts = MFOTL.ts_max; db = LabelledHelper.Db.make_db [] }
           end
         else
           MonpolyError s
