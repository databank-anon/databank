...
module MakeMarshalling (M: MFORMULA) = struct
  ...
  type state = (int * timestamp * bool * M.mformula * (int * timestamp) array * int * int * bool)
      ...
      if not (Dllist.is_empty mezinf.mezauxrels) && not (cell = NEval.void) then
        ...
        if ((tpj - tpq) < 0) then
          let tree_list, ezlast =
            ...
            get_new_elements mezinf.mezauxrels Dllist.void cond f
          in
      ...
      if not(Dllist.is_empty meauxrels) && cell != NEval.void then
        ...
        if ((MFOTL.ts_minus tsj tsq) < 0.0) then
          let tree_list, elast =
            ...
            get_new_elements meinf.meauxrels Dllist.void cond (fun x -> x)
          in
    ...
    let mo2e intv moinf =
      ...
      if not (Dllist.is_empty moauxrels) then
        let tree_list, olast =
          ...
          get_new_elements moauxrels Dllist.void cond (fun x -> x)
        in
    ...
    let moz2e intv mozinf =
      ...
      if not (Dllist.is_empty mozauxrels) then
        ...
        let tree_list, ozlast  =
          ...
          get_new_elements mozauxrels Dllist.void cond f
        in 
