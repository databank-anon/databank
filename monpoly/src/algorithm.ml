...
module type ALGORITHM = sig
  ...
  module L: Log.LOG
  module Filter_rel: Filter_rel.FILTER_REL
...
module MakeAlgorithm (L: Log.LOG) = struct
  ...
  module Helper = Log.H
  module Db = Helper.Db
  module Extformula = Extformula.MakeExtformula(Helper)
  module Filter_rel = Filter_rel.MakeFilter_rel(Helper)
  module Mformula = Mformula.MakeMformula(Extformula)
  module Marshalling = Marshalling.MakeMarshalling(Mformula)
  module Splitting = Splitting.MakeSplitting(Mformula)
  module Hypercube_slicer = Hypercube_slicer.MakeHypercube_slicer(Mformula)
  ...
  let update_since_all intv tsq inf comp rel1 rel2 =
    ...
    if not (Relation.is_empty rel2) then
      ...
      inf.sres <- Relation.adopt inf.sres rel2;
  ...
  let update_old_until q tsq i intv inf discard  =
    ...
    Sj.iter (
        ...
        if (not discard) && not (Relation.is_empty relq) then
          ...
          inf.ures <- Relation.adopt inf.ures relq;
        if Misc.debugging Dbg_eval then
  ...
  let combine2 comp j rels rel2 =
    ...
    Sk.iter
      (fun (k,rel) ->
        ...
        (*if not (Relation.is_empty nrel) then*) (* removed as this risked erasing labels *)
        Sk.add_last (k,nrel) nrels;
        curr_rel2 := Relation.diff !curr_rel2 nrel;
      ) rels;
    (*if not (Relation.is_empty !curr_rel2) then*) (* removed as this risked erasing labels *)
  ...
  let comp_agg_once tsq intv state update_state_old update_state_new get_result rel discard =
    ...
    elim_old_from_timewindow ();
    (*if not (Relation.is_empty rel) then*) (* removed as this risked erasing labels *)
  ...
  let rec eval f neval crt discard =
    ...
    | ENeg f1 ->
       (match eval f1 neval crt discard with
        | Some rel -> Some (Relation.neg rel)
        | None -> None
       )
    ...
    | EOnce (intv,f2,inf) ->
       (match eval f2 neval crt false with
        ...
        | Some rel2 ->
           ...
           (*if not (Relation.is_empty rel2) then*) (* removed as this risked erasing labels *)
         ...
         if neval_is_last neval inf.elastev then
           ...
           if not (MFOTL.in_left_ext (MFOTL.ts_minus tsi tsq) intv) then
             ...
             begin
               ...
               | Some rel2 ->
                  (* we update the auxiliary relations *)
                  if (MFOTL.in_right_ext (MFOTL.ts_minus tsi tsq) intv) then
                    (*if (Relation.is_empty rel2) then*) (* removed as this risked erasing labels *)
    ...
    let rec update lets = function
      | EPred (p, comp, inf) ->
         ...
         then ()
           ...
           let rel =
             (try
                let t = get_table db p in
                Table.get_relation t
              with Not_found ->
                    match Predicate.get_name p with
                    | "tp" -> Relation.make_relation [Tuple.make_tuple [Int i]]
                    | "ts" -> Relation.make_relation [Tuple.make_tuple [Float tsi]]
                    | "tpts" -> Relation.make_relation [Tuple.make_tuple [Int i; Float tsi]]
                    | _ -> Relation.empty
             )
           in
           let rel = comp rel in
      ...
      if not (NEval.is_empty neval) then
        ...
        if tsq < MFOTL.ts_max then
          ...
          | Some rel ->
             ...
             if !Misc.stop_at_first_viol && not (Relation.is_empty_not_null rel) then false
             else eval_loop ()
          | None -> true
        else false
      else true
  ...
  let aggreg_empty_rel ?rel:(rel=Relation.empty) op glist t =
    ...
    if glist = [] then
      begin
        ...
        Relation.make_relation ~rel [Tuple.make_tuple [default_value]]
      end
    else
      Relation.empty
  ...
  let rec add_ext f =
       ...
       let comp =
         if f2_is_special then
           if Misc.subset attr2 attr1 then
             ...
             fun rel1 _ -> Relation.map process_tuple rel1
         else
           match f2 with
           | Neg _ ->
              if attr1 = attr2 then
    ...
    | Aggreg (t_y, y, (Avg as op), x, glist, Once (intv, f))
      ...
      | Aggreg (t_y, y, (Med as op), x, glist, Once (intv, f)) ->
       ...
       let update_state_new_mset (state: Extformula.agg_once_state) rel =
         state.labs <- Labels.LabelSet.union state.labs (Relation.labels rel);
         List.map
           (fun tuple ->
             let gtuple = Tuple.projections posG tuple in
             let gtuple_csts = Tuple.get_constants gtuple in
             let gtuple_labs = Tuple.labels gtuple in
             let cst = Tuple.get_at_pos tuple posx in
             let output = (tuple, gtuple, cst) in
             (try
                let m = Hashtbl.find state.mset tuple in
                Hashtbl.replace state.mset tuple (m+1);
                assert (m > 0);
                assert (Hashtbl.mem state.hres gtuple_csts);
              with Not_found ->
                state.labs <- Labels.LabelSet.union state.labs gtuple_labs;
                Hashtbl.add state.mset tuple 1;
                try
                  let agg_values = Hashtbl.find state.hres gtuple_csts in
                  let _, new_values = update_agg_values true agg_values cst in
                  Hashtbl.replace state.hres gtuple_csts new_values;
                with Not_found ->
                  Hashtbl.add state.hres gtuple_csts (init_agg_values cst));
             output
           ) (Relation.S.elements (Relation.tuples rel))
       in

       (* optimized implementation for unbounded intervals *)
       let update_state_new_all (state: Extformula.agg_once_state) rel =
         state.labs <- Labels.LabelSet.union state.labs (Relation.labels rel);
         Relation.S.iter
           (fun tuple ->
             let gtuple = Tuple.projections posG tuple in
       ...
       let update_state_old (state: Extformula.agg_once_state) rel =
         List.iter (fun (tuple, gtuple, cst) ->
             ...
             if m = 1 then
               begin
                 ...
                 state.labs <- Labels.LabelSet.union state.labs gtuple_labs;
                 if remove then
                   Hashtbl.remove state.hres gtuple_csts
                 else
                   Hashtbl.replace state.hres gtuple_csts new_values
               end
             else
       ...
       let get_result state =
         if Hashtbl.length state.hres = 0 then
           ...
           Hashtbl.fold (fun gtuple agg_values ->
               Relation.add (Tuple.make_tuple ((get_val agg_values)::gtuple)))
             state.hres (Relation.adopt_labels Relation.empty state.labs)
       in
      ...
      | Aggreg (t_y, y, (Max as op), x, glist, Once (intv, f)) ->
       ...
       let get_result state =
         if Hashtbl.length state.tbl = 0 then
           ...
           Hashtbl.fold
             (fun gtuple dllist res ->
               let _, agg_val = Dllist.get_last dllist in
               Relation.add (Tuple.add_first gtuple agg_val) res)
             state.tbl (Relation.adopt_labels Relation.empty state.labs)
       in
    ...
    | Aggreg (t_y, y, Avg, x, glist, f) ->
       ...
       let comp rel =
         if Relation.is_empty rel then
           aggreg_empty_rel ~rel Avg glist TFloat 
         else
           let map = comp_map (Relation.tuples rel) in
           Hashtbl.fold (fun tuple (s,c) rel ->
               let s' = match s with
                 | Float x -> x
                 | Int x -> float_of_int x
                 | _ -> failwith "[Aggreg.Avg, comp] internal error"
               in
               Relation.add (Tuple.add_first tuple (Float (s' /. (float_of_int c)))) rel) map Relation.empty
       in
    ...
    | Aggreg (t_y, y, Med, x, glist, f) ->
       ...
       let comp rel =
         if Relation.is_empty rel then
           aggreg_empty_rel ~rel Med glist TFloat
         else
           let map = comp_map (Relation.tuples rel) in
           Hashtbl.fold (fun tuple (len, vlist) rel ->
               let vlist = List.sort Pervasives.compare vlist in
               let med = Misc.median vlist len fmed in
               Relation.add (Tuple.add_first tuple med) rel) map Relation.empty
       in
       EAggreg (comp, add_ext f)
    ...
    | Aggreg (t_y, y, op, x, glist, f)  ->
       ...
       let comp rel =
         if Relation.is_empty rel then
           aggreg_empty_rel ~rel op glist t_y
         else
           let map = comp_map (Relation.tuples rel) in
           Hashtbl.fold (fun tuple v rel -> Relation.add (Tuple.add_first tuple v) rel) map Relation.empty
       in
       EAggreg (comp, add_ext f)
  ...
  let split_save filename ff closed neval  =
    ...
    let result = split_with_slicer (Hypercube_slicer.add_slices_of_valuation slicer) slicer.degree mf in
  ...
  let rec check_log lexbuf ff closed neval i =
    ...
    let rec loop ffl i =
      ...
      let get_constraints_split_state p = match p with
        ...
        | _ -> raise (Tuple.Type_error ("Unsupported parameter to get_constraints"))
      in
      ...
      | MonpolyCommand {c; parameters} ->
         let process_command = function
           ...
           | _ ->
              ...
              loop ffl i
         in
         process_command c; flush stdout;
