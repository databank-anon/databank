...
let convert_db md =
  ...
  let convert_table t =
    ...
    (name, RBT_set (Relation.S.fold (fun v -> rbt_insert (List.map convert_cst (Tuple.get_constants v)))
                      (Relation.tuples (Table.get_relation t)) rbt_empty)) in
  let db_events = List.map convert_table (get_tables md.db) in
